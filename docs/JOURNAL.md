...
|---|---|---|---|---|
| 25/09/2026 | 0.6.1 | <Nom, prénom> | Fonctionnel | Issue #6 — EF5 dépôt d'exercice : 201 avec statut initial, un seul exercice par étudiant/session, refus 409 si session clôturée (RG10), 400 si en-tête d'identification manquant. Contrôleur aligné sur le corps plat du contrat (ExerciceDepotInput). |
| 25/09/2026 | 0.6.0 | <Nom, prénom> | Fonctionnel | Issue #5 — EF1 ouverture de session : code unique généré, expiration à ouverture + 15 min (RG1), tests d'acceptation. Réparation des incohérences de fusion (CodeErreur/RegleMetierException repositionnés dans exception). |
